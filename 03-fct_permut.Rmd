...
data("mtcars")
corr = mtcars[,c(3,6)]
#cor(corr$disp,corr$wt)
shake = sample(size = length(corr$disp), replace = F, x = corr$disp)
new_corr = cbind(shake,corr[,2])
new = as.data.frame(new_corr)

par(mfrow = c(1,2))
plot(corr$disp,corr$wt, xlab = "disp", ylab = "wt")
plot(new$shake,new$V2, xlab = "disp", ylab = "wt")
text(new, row.names(new), cex=0.65, pos=1,col="red") 
...
```{r, message=FALSE, warning = FALSE, fig.width=5, fig.height=3, fig.cap = "Graphique d'une Permutation Features Importance pour un arbre décisionnel appliqué au jeu de données iris."}
data("iris")
...
imp <- FeatureImp$new(mod, loss = "mse", n.repetitions = 100)
...
```{r, warning=FALSE, message=FALSE, fig.width=5, fig.height=3,fig.cap = "Graphique d'une Permutation Features Importance pour une forêt aléatoire appliquée au jeu de données iris."}
fa_1 = randomForest(Petal.Length~., 
                       data = iris, 
                       importance = TRUE)
...
mod_1 <- Predictor$new(fa_1, 
                     data = X, 
                     y = Y)

imp_1 <- FeatureImp$new(mod_1, loss = "mse", n.repetitions = 100)
...
```{r, fig.width=12, fig.height=5, fig.cap = "Graphique de deux Permutation Features Importance pour une forêt aléatoire ainsi que pour un arbre décisionnel appliqués au jeu de données fifa."}
fifa = read.csv(file = "complete.csv", header = T, sep = ",")
fifa = fifa[1:5000,18:30]
fifa = na.exclude(fifa)
attach(fifa)

Y_2 <- fifa$overall
X_2 <- fifa[-which(names(fifa) == "overall")]

tree_t = rpart(formula = overall ~ ., data = fifa)
mod_t <- Predictor$new(tree_t, data = X_2, y = Y_2)
imp_t <- FeatureImp$new(mod_t, loss = "mse")

fa_f = randomForest(formula = overall ~ .,data = fifa)
mod_f<- Predictor$new(fa_f, data = X_2, y = Y_2)
imp_f <- FeatureImp$new(mod_f, loss = "mse")

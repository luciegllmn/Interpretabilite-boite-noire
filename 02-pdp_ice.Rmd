...
data('weather')
weather2 = weather %>% select(origin,temp, dewp, humid, wind_dir, wind_speed, wind_gust, precip, pressure, visib)
weather2 = weather2 %>% drop_na() #on a supprimer les lignes pour lesquelles il y a des NA
```

```{r}
airport = fct_recode(weather2$origin, 
                      "Aiport EWR" = "EWR",
                      "Airport JFK" = "JFK",
                      "Airport LGA" = "LGA")
weather2 = mutate(weather2, airport)
weather2 = weather2[,-1]
```

#### Construction du PDP avec comme boite noire une forêt aléatoire {-}
```{r, message = FALSE, warning=FALSE, fig.height=5, fig.width=10, fig.cap="PDP pour le modèle de prédiction de la température dans les aéroports de New York avec la méthode des forêts aléatoires."}
algo_FA = randomForest(temp~., 
                       data = weather2, 
                       importance = TRUE)

predictor_FA = Predictor$new(model = algo_FA, 
                          data = weather2)

pdp_FA = FeatureEffect$new(predictor_FA, 
                        feature = c("dewp", "humid", "wind_dir", "wind_speed", "wind_gust",
                                    "precip", "pressure", "visib", "airport"), 
                        method = "pdp", 
                        grid.size = 30)

pdp_FA$set.feature("dewp")
k1 = pdp_FA$plot() +
  scale_y_continuous('Prédiction de la température (en F)') +
  scale_x_continuous('Point de rosée')

pdp_FA$set.feature("humid")
k2 = pdp_FA$plot()+
  scale_y_continuous('Prédiction de la température(en F)') +
  scale_x_continuous('Humidité') 

pdp_FA$set.feature("wind_speed")
k3 = pdp_FA$plot()+
  scale_y_continuous('Prédiction de la température (en F)') +
  scale_x_continuous('Vitesse du vent (en mph)')

pdp_FA$set.feature("airport")
k4 = pdp_FA$plot() +
  scale_y_continuous('Prédiction de la  température (en F)', limits = c(0,55))+
  scale_x_discrete("Aéroport")

gridExtra::grid.arrange(k1, k2, k3,k4 ,ncol = 4)
```
...
```{r, warning=FALSE, message=FALSE, fig.cap = "PDP de la prédiction de la température et l'intéraction de l'humidité et de la vitesse du vent.", fig.height=5, fig.width=8, fig.align="center"}
pd = FeatureEffect$new(predictor_FA, 
                       feature = c("humid", "wind_speed"), 
                       method = "pdp")

pd$plot() +
  scale_fill_viridis(option = "D") +
  labs(x = 'Humidité', y = 'Vitesse du vent', fill = "Prédication de la température (F)")
```
...
```{r, message = FALSE, warning=FALSE, fig.height=5, fig.width=10, fig.cap="PDP pour le modèle de prédiction de la température dans les aéroports de New York avec la méthode SVM."}
SVM = svm(temp~.,
          data = weather2)

predictor = Predictor$new(model = SVM, 
                          data = weather2)
...
gridExtra::grid.arrange(l1, l2, l3,l4 ,ncol = 4)
```
...
#### Construction du PDP avec comme boite noire une forêt aléatoire {-}
```{r, warning = FALSE, echo = FALSE, message = FALSE, fig.cap="PDP pour le modèle de prédiction du nombre de cylindres d'une voiture en fonction de son poids, son déplacement, sa puissance et le rapport du pont arrière avec la méthode des forêts aléatoires.", fig.height=5, fig.width=10}
mtcars2 = mtcars %>% select("mpg", "cyl", "disp", "hp", "drat", "wt")

algo_FA = randomForest(as.factor(cyl)~., 
                       data = mtcars2, 
                       importance = TRUE)

predictor = Predictor$new(model = algo_FA,
                          data = mtcars2)

pdp = FeatureEffect$new(predictor, 
                        feature = c("wt", "disp", "hp", "drat", "mpg"), 
                        method = "pdp", 
                        grid.size = 30)

pdp$set.feature("wt")
p1 = pdp$plot() +
  scale_y_continuous('Prédiction du nombre de cyclindres') +
  scale_x_continuous('Poids')

pdp$set.feature("disp")
p2 = pdp$plot()+
  scale_y_continuous('Prédiction du nombre de cyclindres') +
  scale_x_continuous('Volume du moteur') 

pdp$set.feature("hp")
p3 = pdp$plot()+
  scale_y_continuous('Prédiction du nombre de cyclindres') +
  scale_x_continuous('Puissance')
...
####Construction du PDP avec comme boite noire un algorithme SVM{-}
```{r, warning = FALSE, echo = FALSE, message = FALSE, fig.height=5, fig.width=10, fig.cap="PDP pour le modèle de prédiction du nombre de cylindres d'une voiture en fonction de son poids, son déplacement, sa puissance et le rapport du pont arrière avec la méthode SVM."}
SVM = svm(as.factor(cyl)~.,
          data = mtcars2)
...
m2 = pdp$plot()+
  scale_y_continuous('Prédiction du nombre de cyclindres') +
  scale_x_continuous('Déplacement') 
...
gridExtra::grid.arrange(m1, m2, m3, m4, ncol = 4)
```
...
```{r, message = FALSE, warning = FALSE, fig.height=5, fig.width=10, fig.cap = 'ICE pour le modèle de prédiction de la température (en F) avec la méthode des forêts aléatoires.'}
algo_FA = randomForest(temp~., 
                       data = weather2, 
                       importance = TRUE)
...
gridExtra::grid.arrange(k1, k2, k3 ,ncol = 4)
```

...
```{r, message = FALSE, warning = FALSE}
data(weather)
weather2 = weather %>% select(temp, dewp, humid, wind_dir, wind_speed, wind_gust, precip, pressure, visib)
weather2 = weather2 %>% drop_na() #on a supprimer les lignes pour lesquelles il y a des NA
algo_FA = randomForest::randomForest(temp~., data = weather2, importance = TRUE)
```

```{r, message=FALSE, warning = FALSE, fig.width=7, fig.height=3,  fig.cap = "Graphique LIME pour la prédiction de la température des aéroports de New York avec une forêt aléatoire."}
predictor = Predictor$new(model = algo_FA,
                          data = weather2)

lime.explain = LocalModel$new(predictor, x.interest = weather2[3,], k = 8) #k règle le nb de variable
...
```{r, fig.width=7, fig.height=3, fig.cap = "Graphique LIME pour la prédiction de la température des aéroports de New York avec l'algorithme des KNN."}
knn <- train(temp~., data=weather2, method="knn", verbose = FALSE)

predictor2 = Predictor$new(model = knn,
                          data = weather2)

lime.explain2 = LocalModel$new(predictor2, x.interest = weather2[1,], k = 8) #k règle le nb de variable
...
####Construction de LIME avec comme boite noire une forêt aléatoire {-}   
```{r, message = FALSE, warning=FALSE, fig.height=3, fig.width=7, fig.cap = "Graphique LIME pour la prédiction de la longueur des sépales d'une fleur avec une forêt aléatoire."}
data('iris')
iris2 = iris[,1:4]
iris.rf <- randomForest(Sepal.Length~., data = iris2)

predictor = Predictor$new(model = iris.rf,
                          data = iris2)

lime.explain = LocalModel$new(predictor, x.interest = iris2[1,], k = 3) #k règle le nb de variable
...
####Construction de LIME avec comme boite noire l'agorithme des KNN {-} 
```{r,fig.height=3, fig.width=7, fig.cap = "Graphique LIME pour la prédiction de la longueur des sépales d'une fleur avec l'algorithme des KNN."}
knn <- train(Sepal.Length~., data = iris2, method="knn", verbose = FALSE)

predictor2 = Predictor$new(model = knn,
                          data = iris2)

lime.explain2 = LocalModel$new(predictor2, x.interest = iris2[1,], k = 3) #k règle le nb de variable
